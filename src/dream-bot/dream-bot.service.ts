...
export class DreamBotService {
  ...
  private async runSingleCycle(): Promise<void> {
    try {
      // เข้าสู่หน้าเว็บ
      await this.page.goto('https://dream2number.com', { waitUntil: 'networkidle2' });
    ...
    try {
      ...
      await this.page.waitForFunction(() => {
        ...
        const backButton = Array.from(document.querySelectorAll('button, a'))
          .find(btn => btn.textContent?.includes('กลับ') || btn.textContent?.includes('อื่น'));
  ...
  private async clickBackButton(): Promise<void> {
    try {
      ...
      const backButtonClicked = await this.page.evaluate(() => {
        ...
        if (backLinkByHref && backLinkByHref.textContent?.includes('กลับ')) {
